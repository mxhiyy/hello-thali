...
const sendOTP = async (req, res, next) => {
    ...
    try {
        ...
        //set OTP exipry time to 1 min
        const otpExpires = new Date(Date.now() + 35 * 1000);

        //send the OTP and using the Bitts Service
        const options = {
            method: 'POST',
        ...
        request(options, async (error, response) => {
            ...
            //save user with hashed otp and message id
...
const verifyOTP = async (req, res, next) => {
    ...
    try{
        ...
        res.json({ message: 'OTP verified Succesfully',  token: jwt.generateToken(user._id) });
